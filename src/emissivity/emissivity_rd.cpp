...
int main(int argc, char** argv)
{
	...
	char default_par_filename[] = "../par/emissivity_rd.par";
	char *par_filename;
	...
	// Use a FlatDiscDestination to stop rays when they cross theta_lim.
	// Replace this with any RayDestination subclass to trace to a different geometry.
	...
	for (int ray = 0; ray < raytrace_source.get_count(); ray++)
	{
		...
		{
		    const double r     = raytrace_source.rays[ray].r;
		    const double theta = raytrace_source.rays[ray].theta;

		    // Ray reached the disc if it stopped at theta_lim and is outside the ISCO
		    if(theta >= theta_lim - 1e-3 && raytrace_source.rays[ray].redshift > 0 && r >= r_isco)
            {
                int ir = (logbin_r) ? static_cast<int>( log(r / r_min) / log(dr)) : static_cast<int>((r - r_min) / dr);
