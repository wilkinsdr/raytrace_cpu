//
// imageplane_disc_image_isco.cpp
//
// Ray-traced disc image using DiscWithISCODestination to stop rays on the
// equatorial plane only at r >= ISCO.  Rays that cross theta = pi/2 inside
// the ISCO are allowed to continue to the horizon rather than being
// incorrectly accepted as disc hits.
//
...
int main(int argc, char **argv)
{
    ...
    string par_filename = par_args.key_exists("--parfile")
                          ...
                          : "../par/imageplane_disc_image_isco.par";
    ...
    {
        cerr << "Warning: Euler integrator does not support RayDestination stopping conditions; using RK45" << endl;
    ...
    fits.create_primary();
    fits.write_comment("Raytraced images of accretion disc (DiscWithISCODestination)");
    fits.write_keyword("GENERATOR", "Simulation results were generated by this software", "imageplane_disc_image_isco");
