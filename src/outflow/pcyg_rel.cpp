...
double los_vel_z(double z, void* p)
{
    ...
    const double mu = z /sqrt((params->p)*(params->p) + z*z);
//    return w * z /sqrt((params->p)*(params->p) + z*z) - params->v;

    const double dopp = (1. + params->vinf*w*mu) / sqrt(1. - params->vinf*params->vinf*w*w);
    return (dopp - 1)/params->vinf - params->v;
}
...
double tau(double z, void* par)
{
    ...
    const double gamma = 1./ sqrt(1. - params->vinf*params->vinf*w*w);
...
//        return tau0(sqrt(p * p + z0 * z0), params);
 ...
 //       return profile;
        ...
        return profile * (tau0(sqrt(p * p + z0 * z0), params)*w/r) /
                (mu * params->vinf * gamma*gamma*gamma * dwdr +
                gamma * (w/r) * (1. + s*mu*mu));

        return profile * tau0(sqrt(p * p + z0 * z0), params) / (gamma * (1. + sigma(p * p + z0 * z0, params) * mu * mu));
...
double flux_integrand(double p, void* par)
{
    ...
    const double dopp = (1. - params->vinf*params->v) / sqrt(1. - params->vinf*params->vinf*w*w);
    const double dopp_source = 1; //sqrt((1. - w*params->vinf)/(1. + w*params->vinf));
    const double gamma = 1./ sqrt(1. - params->vinf*params->vinf*w*w);
 //   cout << r << " " << dopp_source << endl;
...
//                              * source_func(r, params);
  ...
  //      return dwzdz;
        ...
        {
            return 2 * p * (dopp*dopp*dopp*source*dopp_source*dopp_source*dopp_source + exp(-1 * tau_star));
        }
        else
        {
            return 2 * p * dopp*dopp*dopp*source*dopp_source*dopp_source*dopp_source;
        }
    }
    else
    {
...
int main()
{
    ...
    TextOutput outfile("../dat/pcyg_sei_v05.dat");
...
//    for(double wmu = -1; wmu<=0; wmu += 0.1)
